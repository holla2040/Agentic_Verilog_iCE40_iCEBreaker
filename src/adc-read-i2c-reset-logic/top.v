// ============================================================================
// ADS1115 I2C ADC Reader - Top Level Module (with Synchronous Reset)
// ============================================================================
//
// Reads 16-bit ADC values from ADS1115 and outputs via UART.
// Includes button-triggered synchronous reset for system recovery.
//
// OPERATION:
//   1. Sends "\r\nads1115\r\n" on startup via UART
...
//
// RESET BEHAVIOR:
//   - Button 3 (pin 18, active HIGH) triggers synchronous reset
//   - 2-stage synchronizer prevents metastability
//   - On reset: I2C lines released (bus recovery), UART TX idle (HIGH)
//   - System restarts from ST_STARTUP state
//
// HARDWARE:
//   - iCEBreaker FPGA (12 MHz clock)
//   - ADS1115 on I2C (SCL=pin 45, SDA=pin 47)
//   - Button on pin 20 -> ADS1115 ADDR pin
//   - Reset button on pin 18 (active HIGH)
...
module top (
    ...
    // Reset input
    input  wire rst_i,         // Reset button (active HIGH)
    ...
    // ========================================================================
    //
    // The reset button is an asynchronous input that can change at any time.
    // Sampling an asynchronous signal directly can cause metastability -
    // the flip-flop may enter an undefined state if the input changes too
    // close to the clock edge.
    //
    // A 2-stage synchronizer reduces metastability probability to negligible
    // levels by giving the first flip-flop time to resolve before the second
    // samples it. This adds 2 clock cycles of latency (167ns at 12 MHz).

    reg rst_sync_0 = 1'b0;     // First synchronizer stage
    reg rst_sync_1 = 1'b0;     // Second synchronizer stage (use this!)
    ...
    wire rst = rst_sync_1;     // Synchronized reset signal
    ...
    // ========================================================================
    //
    // Named constants for hex-to-ASCII conversion improve readability
    // and make the code more self-documenting.

    localparam ASCII_0 = 8'h30;  // '0' = 0x30
    localparam ASCII_A = 8'h41;  // 'A' = 0x41
    ...
    function [7:0] hex_to_ascii;
        ...
        begin
            if (nibble < 10)
                hex_to_ascii = ASCII_0 + nibble;       // '0' + value
            else
                hex_to_ascii = ASCII_A + (nibble - 10); // 'A' + (value - 10)
        end
    endfunction
    ...
    uart_tx #(
        ...
        .clk(clk),
        .rst(rst),               // Connect synchronized reset
        .data_i(uart_data),
        .start_i(uart_start),
        .busy_o(uart_busy),
        .tx_o(uart_tx)
    ...
    always @(posedge clk) begin
        ...
        // SYNCHRONOUS RESET
        // ----------------------------------------------------------------
        // On reset: return to startup state
        // All control signals cleared to prevent spurious operations
        // Sub-modules (UART, I2C) handle their own reset behavior
        if (rst) begin
            state <= ST_STARTUP;
            msg_index <= 4'd0;
            uart_start <= 1'b0;
            ...
            case (state)
                ...
                // Sequence: START -> 0x90 -> 0x01 -> 0xC2 -> 0xC3 -> STOP
                // ================================================================
